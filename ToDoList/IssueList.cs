...
namespace ToDoList
{
    ...
    public class IssueList
    {
        ...
        internal void Delete(int selectedIssueNumber)
        {
            ...
            if (issueIndexToDelete == (issueCount - 1))
            {
                _issues[issueIndexToDelete] = null;
            }

            else if (issueCount == _issues.Length)
            {
                Issue[] _newIssues = new Issue[_issues.Length+2];
                _newIssues = _issues;

                for (int i = issueIndexToDelete; i < issueCount; i++)
                {
                    {
                        _newIssues[i] = _newIssues[i + 1];
                    }

                    _newIssues[issueCount] = null;
                    _issues = _newIssues;
                }
